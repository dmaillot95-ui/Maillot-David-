...
# Same synthetic deterministic stress harness as rung 125.
# Candidate and sham get identical per-task budgets; only allocation policy differs.
# This does not model real LLM context/message overhead.
def make_tasks(seed):
...
def score_task(t, policy, rng):
    base_budget=1.0; coord=0.18; verify_budget=0.30
...
for seed in SEEDS:
    ...
    rows.append({"seed":seed,"baseline":b,"matched_sham":s,"candidate_200":c})

def mean(arm,key): return statistics.mean(r[arm][key] for r in rows)

base_res=mean("baseline","residual"); cand_res=mean("candidate_200","residual"); sham_res=mean("matched_sham","residual")
base_v=mean("baseline","verified_work"); cand_v=mean("candidate_200","verified_work"); sham_v=mean("matched_sham","verified_work")
base_g=mean("baseline","vcgc"); cand_g=mean("candidate_200","vcgc"); sham_g=mean("matched_sham","vcgc")
cb=sum(r["candidate_200"]["residual"]<r["baseline"]["residual"] and r["candidate_200"]["verified_work"]>r["baseline"]["verified_work"] for r in rows)
cs=sum(r["candidate_200"]["residual"]<r["matched_sham"]["residual"] and r["candidate_200"]["verified_work"]>r["matched_sham"]["verified_work"] for r in rows)
fam_ok=0
for fam in FAMILIES:
    c=statistics.mean(r["candidate_200"]["families"][fam]["vcgc"] for r in rows)
    s=statistics.mean(r["matched_sham"]["families"][fam]["vcgc"] for r in rows)
    if c>=0.98*s: fam_ok+=1
resource_parity=max(abs(r["candidate_200"]["cost"]-r["matched_sham"]["cost"]) for r in rows)<1e-12
...
summary={
 ...
 "means":{"baseline":{"residual":base_res,"verified_work":base_v,"vcgc":base_g},"candidate_200":{"residual":cand_res,"verified_work":cand_v,"vcgc":cand_g},"matched_sham":{"residual":sham_res,"verified_work":sham_v,"vcgc":sham_g}},
 ...
 "claim_ceiling":"synthetic deterministic benchmark only; no real LLM-message overhead modeled"
}
Path("artifacts").mkdir(exist_ok=True)
Path("artifacts/c45_rung_0200_results.json").write_text(json.dumps({"summary":summary,"rows":rows},indent=2),encoding="utf-8")
