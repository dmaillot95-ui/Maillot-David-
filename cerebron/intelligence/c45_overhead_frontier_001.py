...
SCALES=[50,75,100,125,150,175,200,225,250,300]
...
# Synthetic deterministic overhead frontier. This is not an LLM benchmark.
...
def run(seed,n,policy):
    ...
    for t in tasks:
        ...
        # same size-dependent context/fusion penalty for both arms
        q=max(0.0,q-ctx)
        fam,d,a,v,s=t
        residual=(1-q)*(1+d+a)
        verified=q*(1+0.75*v)
    ...
    # net useful work charges coordination/verification/fusion overhead in work-equivalent units
...
passing=[x for x in summaries if x["gate"]=="PASS"]
best=max(passing,key=lambda x:x["candidate"]["net_vcgc"]) if passing else None
result={
 "id":"C45-OVERHEAD-FRONTIER-001",
 "summaries":summaries,
 "best_operating_point":None if best is None else best["scale"],
 "best_net_vcgc":None if best is None else best["candidate"]["net_vcgc"],
 "claim_ceiling":"synthetic deterministic overhead model only; not actual LLM message throughput"
}
Path("artifacts").mkdir(exist_ok=True)
Path("artifacts/c45_overhead_frontier_001_results.json").write_text(json.dumps({"result":result,"rows":rows},indent=2),encoding="utf-8")
