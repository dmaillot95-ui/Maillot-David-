...
# Exact frozen model from C45-OVERHEAD-FRONTIER-001; only scale 60 is inserted.
...
result={
 "id":"C45-OVERHEAD-FRONTIER-0060",
 "summaries":summaries,
 "claim_ceiling":"synthetic deterministic overhead model only; not actual LLM message throughput"
}
Path("artifacts").mkdir(exist_ok=True)
Path("artifacts/c45_overhead_frontier_0060_results.json").write_text(json.dumps({"result":result,"rows":rows},indent=2),encoding="utf-8")
